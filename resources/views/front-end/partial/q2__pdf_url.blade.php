        ...
        @if(auth()->user()->canDownload())
            <td>
                <form action="{{ route('download.store' )}}" method="post" style="display: inline;">
                    @csrf
                    <input name="file_path" type="hidden" value="{{ env('S3_URL') }}{{ $item->q2__pdf_url }}">
                    <button type="submit" class="btn btn-outline-primary">{{ $item->q2__pdf_url_file_name != null ? $item->q2__pdf_url_file_name : 'PDF' }}</button>
                </form>
            </td>
        @else
